...
#      'aman',
    ...
    if 'llama-3' in args.model_name:
        verbalizerfile=f'/mnt/e72cc1a3-e45e-4a0c-b1e6-0e4357d2e753/zixiao/RAG/label-words-are-anchors-main/results/verbalizer/basedonlogits/filter_low_quality/basedontrain/{args.task_name}_verbalizer_llama3_movelogit.json'
    else:
        verbalizerfile=f'/mnt/e72cc1a3-e45e-4a0c-b1e6-0e4357d2e753/zixiao/RAG/label-words-are-anchors-main/results/verbalizer/basedonlogits/filter_low_quality/basedontrain/{args.task_name}_verbalizer_llama3_movelogit.json'
    ...
    filter_pointbiserialr={}
    for r in args.label_dict:
        if r not in filter_pointbiserialr:
            filter_pointbiserialr[r]= {}
        token=label_map_origin[r]
        verbalzier_words=verbalizers[str(r)]
        binary_data = fulllabel.tolist()
        for l in range(len(binary_data)):
            if binary_data[l]!=r:
                binary_data[l]=0
            else:
                binary_data[l]=1
        # if r==0:
        #     flipped_list = [1 - x for x in binary_data]
        #     binary_data=flipped_list
        for w in range(len(verbalzier_words)):
            token = label_map_origin[r][verbalzier_words[w]][0]
            continuous_data =fulllogits[:,token].tolist()
            r_pb, p_value = pointbiserialr(binary_data, continuous_data)
            filter_pointbiserialr[r][verbalzier_words[w]]={'r_pb':r_pb,'p_value':p_value}
        print(f'{args.task_name}: {r} has {len(filter_pointbiserialr[r])}')
    '''save all pb value based on negative and positive'''

    with open(f'/mnt/e72cc1a3-e45e-4a0c-b1e6-0e4357d2e753/zixiao/RAG/label-words-are-anchors-main/results/verbalizer/basedonlogits/filter_low_quality/basedontrain/{args.task_name}_pointbiserialr_llama3.json', 'w') as json_file:
        json.dump(filter_pointbiserialr, json_file)


    '''filter out the words with r_pb<0 then p_value>0.05'''
    deep_copied_dict = copy.deepcopy(filter_pointbiserialr)
    for l in filter_pointbiserialr:
        for w in filter_pointbiserialr[l]:
            rpb=filter_pointbiserialr[l][w]['r_pb']
            pvalue=filter_pointbiserialr[l][w]['p_value']
            if rpb<0:
                del deep_copied_dict[l][w]
                print(f'{w} r_pb={rpb}<0')
            if rpb>0 and pvalue>0.05:
                del deep_copied_dict[l][w]
                print(f'{w} r_pb={rpb}>0, p={pvalue}>0.05')
        print(f'{args.task_name} filtered: {l} has {len(deep_copied_dict[l])}')
    with open(
            f'/mnt/e72cc1a3-e45e-4a0c-b1e6-0e4357d2e753/zixiao/RAG/label-words-are-anchors-main/results/verbalizer/basedonlogits/filter_low_quality/basedontrain/{args.task_name}_filter_pointbiserialr_llama3.json',
            'w') as json_file:
        json.dump(deep_copied_dict, json_file)

    filter_verbalizer={key:[] for key in args.label_dict}

    for l in deep_copied_dict:
        sorted_keys = sorted(deep_copied_dict[l], key=lambda x: deep_copied_dict[l][x]['r_pb'], reverse=True)

        # Creating a new ordered dictionary based on the sorted keys
        sorted_dict = {key: deep_copied_dict[l][key] for key in sorted_keys}
        for w in sorted_dict:
            filter_verbalizer[l].append(w)
    with open(
            f'/mnt/e72cc1a3-e45e-4a0c-b1e6-0e4357d2e753/zixiao/RAG/label-words-are-anchors-main/results/verbalizer/basedonlogits/filter_low_quality/basedontrain/{args.task_name}_verbalizer_movelogit_rpb_llama3.json',
            'w') as json_file:
        json.dump(filter_verbalizer, json_file)
