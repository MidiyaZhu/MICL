...
class RetrieverDatasetReader(torch.utils.data.Dataset):
    ...
    def __init__(self, model_name, task_name, index_split, dataset_path, n_tokens=700,
        ...
        if task_name in ['isear','aman','cr','trec','sst2','imdb','ag_news']:
            self.dataset_wrapper = get_dataset_wrapper(task_name)(dataset_split='retrieve',dataset_path=dataset_path)
        else:
