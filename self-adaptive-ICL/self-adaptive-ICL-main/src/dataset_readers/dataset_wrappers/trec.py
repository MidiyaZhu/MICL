...
def get_Y_TEXT(entry):
    # return label2text[entry['coarse_label']] if 'Y_TEXT' not in entry.keys() else entry['Y_TEXT']
...
def get_Y(entry):
    # return entry['coarse_label'] if 'Y' not in entry.keys() else entry['Y']
    return entry['label'] if 'Y' not in entry.keys() else entry['Y']


@field_getter.add("ALL")
def get_ALL(entry):
    # return f"{entry['text']}\tIt is {label2text[entry['coarse_label']]}" if 'ALL' not in entry.keys() else entry['ALL']
...
class TRECDatasetWrapper(DatasetWrapper):
    ...
    def __init__(self, dataset_path=None, dataset_split=None, ds_size=None):
        def _abs_label(ex):
            # print([key for key in ex])
            # ex['coarse_label'] = abs(ex['coarse_label'])
        ...
        if dataset_split == 'test':
            ...
            # dftest = pd.read_csv('/mnt/e72cc1a3-e45e-4a0c-b1e6-0e4357d2e753/zixiao/RAG/self-adaptive-ICL/self-adaptive-ICL-main/data/valid/valid_isear.csv')  # replace with your actual path
            dftest = pd.read_csv(dataset_path)  # replace with your actual path
