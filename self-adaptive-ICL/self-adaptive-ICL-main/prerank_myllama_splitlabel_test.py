...
class PreRank:
    ...
    def forward(self, dataloader, **kwargs):
        ...
        for i, entry in enumerate(dataloader):
            with torch.no_grad():
                ...
                raw_text = self.retriever_model.tokenizer.batch_decode(entry['input_ids'], skip_special_tokens=True)  #'raw test samples in words'  64
                res = self.retriever_model.encode(raw_text, show_progress_bar=False, **kwargs) #64,768
            res_list.extend([{"embed": r, "metadata": m} for r, m in zip(res, metadata)]) #133, each one is a dict with embedding vector and idx in test sample
        return res_list

    def knn_search(self, entry, num_candidates=1, num_ice=1):
        embed = np.expand_dims(entry['embed'], axis=0) #[1,768]
        near_ids = self.index.search(embed, max(num_candidates, num_ice) + 1)[1][0].tolist()  #return knn results index (ordered
    ...
    def find(self):
        res_list = self.forward(self.dataloader)  #embedding vector of each test senetence
        data_list = []
        starttime = datetime.datetime.now()
    ...
    for task_name in [
       ...
       'ag_news',
        ...
        for key in label_dict:
            ...
            cfg.output_file=retrieve_file
            cfg.emb_field =emb_field
            cfg.num_ice =num_ice
            cfg.method =prerank_method
            cfg.num_candidates =num_candidates
            cfg.dataset_reader.task_name =task_name
            cfg.rand_seed =rand_seed
            cfg.dataset_reader.dataset_split =dataset_split
            cfg.index_reader.task_name =task_name
            cfg.index_file =os.path.join(run_dir, f'index{key}')
            cfg.scale_factor = 0.1
            '''valid'''
            # cfg.index_reader.dataset_path =f'/mnt/e72cc1a3-e45e-4a0c-b1e6-0e4357d2e753/zixiao/RAG/label-words-are-anchors-main/data/new/train/train_{task_name}_{seed}_{key}.csv'
            # cfg.dataset_reader.dataset_path = f'/mnt/e72cc1a3-e45e-4a0c-b1e6-0e4357d2e753/zixiao/RAG/label-words-are-anchors-main/data/new/valid/valid_{task_name}_{seed}.csv'
            '''test'''
            cfg.dataset_reader.dataset_path = f'/mnt/e72cc1a3-e45e-4a0c-b1e6-0e4357d2e753/zixiao/RAG/label-words-are-anchors-main/data/new/test/test_{task_name}.csv'
            cfg.index_reader.dataset_path =f'/mnt/e72cc1a3-e45e-4a0c-b1e6-0e4357d2e753/zixiao/RAG/label-words-are-anchors-main/data/new/train/train_{task_name}_1000_{key}.csv'
