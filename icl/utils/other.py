...
def sample_two_set_with_per_shot_per_class(ori_data, a_shot, b_shot, seed, label_name: str = 'labels',
                                       a_total_shot=None, b_total_shot=None):
    a_label_count = {}
    b_label_count = {}
    a_data_idx = {}
    ...
    for index in all_indices:
        ...
        if a_label_count[label] < a_shot:
            if label not in a_data_idx:
                a_data_idx[label]=[index]
            else:
                a_data_idx[label].append(index)
            # a_data_idx.append(index)
            a_label_count[label] += 1
            a_total_cnt += 1
        elif b_label_count[label] < b_shot:
            # if label not in b_data_idx:
            #     b_data_idx[label] = [index]
            # else:
                # b_data_idx[label].append(index)
            b_data_idx.append(index)
            b_label_count[label] += 1
            b_total_cnt += 1
    ...
    a_data={}
    b_data={}
    for split in range(len(a_data_idx[0])):
        temp_a=[]
        for l in a_data_idx:
            temp_a.append(a_data_idx[l][split])
        a_data[split] = ori_data.select(temp_a)
    # for split in range(len(b_data_idx[0])):
    #     temp_b = []
    #     for l in b_data_idx:
    #         temp_b.append(b_data_idx[l][split])
    #     b_data[split] = ori_data.select(temp_b)
    b_data = ori_data.select(b_data_idx)
    return a_data, b_data
