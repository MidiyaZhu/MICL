...
class LMForwardAPI(nn.Module):
    def __init__(self,multi_token, model, model_name, tokenizer, label_dict: Dict[int, str], device='cuda:0'):
        super().__init__()
        ...
        if 'llama-2' in model_name.lower():
            self.label_map = {tokenizer.encode(v, add_special_tokens=False)[1]: k for k, v in label_dict.items()}
            self.label_map_tokens ={k : tokenizer.encode(v, add_special_tokens=False)[1:] for k, v in label_dict.items()}
        else:
            self.label_map = {tokenizer.encode(v, add_special_tokens=False)[0]: k for k, v in label_dict.items()}
            self.label_map_tokens = {k: tokenizer.encode(v, add_special_tokens=False) for k, v in label_dict.items()}
        self.position_offset = 0

        assert model_name in ['gpt2-xl', 'gpt-j-6b','EleutherAI/gpt-j-6B','meta-llama/Llama-2-7b-chat-hf','meta-llama/Llama-2-13b-chat-hf','meta-llama/Meta-Llama-3-8B']
    ...
    def cal_logits(self, inputs, **kwargs):
        ...
        if self.use_past_key_values:
            ...
            if self.model_name in ['gpt-j-6b','EleutherAI/gpt-j-6B','gpt2-xl','meta-llama/Llama-2-7b-chat-hf','meta-llama/Meta-Llama-3-8B']:
    ...
    def _cal_probs_tokens(self, logits):
        for c in self.label_map_tokens:
            interest_index = list(self.label_map_tokens[c])
            # logit_c = logits[:, interest_index].sum(dim=1)
            # logit_c = logits[:, interest_index].max(dim=1).values
            logit_c = logits[:, interest_index].mean(dim=1)
            try:
                logits_c = torch.cat((logits_c, logit_c))
            except:
                logits_c = logit_c
    ...
    def forward(self, **kwargs):
        ...
        if self.multi_token == False:
            probs, logits = self._cal_probs(ori_logits)
        else:
            probs, logits = self._cal_probs_tokens(ori_logits)
...
class llamaLMForwardAPI(nn.Module):
    def __init__(self, model, model_name, tokenizer, label_dict: Dict[int, str], device='cuda:0'):
        ...
        assert model_name in ['gpt2-xl', 'gpt-j-6b','EleutherAI/gpt-j-6B','meta-llama/Llama-2-7b-chat-hf','meta-llama/Meta-Llama-3-8B']
    ...
    def cal_logits(self, inputs, **kwargs):
        ...
        if self.use_past_key_values:
            ...
            kwargs['past_key_values'] = past_key_values
            inputs['attention_mask'] = self.get_mask_with_past_key_values(inputs['attention_mask'])
            if self.model_name in ['gpt-j-6b','gpt2-xl','EleutherAI/gpt-j-6B','meta-llama/Llama-2-7b-chat-hf','meta-llama/Meta-Llama-3-8B']:
                bsz, sql = inputs['input_ids'].shape
                position_ids = torch.arange(sql, dtype=torch.long, device=self.device).repeat(bsz, 1)
                position_ids = position_ids + self.position_offset
                kwargs['position_ids'] = position_ids
