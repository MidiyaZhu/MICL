...
class AttentionerManagerBase:
    ...
    def grad_process(self, grad,use_abs = True):
        assert len(grad.shape) == 4  #[1,32,524,524]
        grad = grad.sum(1) #[1,524]
        if use_abs:
            grad = abs(grad)
        return grad
...
def llama2_self_attn( self, hidden_states: torch.Tensor,
        ...
        attn_output = attn_output.reshape(bsz, q_len, self.hidden_size)  #[1,524,4096]
