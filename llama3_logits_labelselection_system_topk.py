...
def build_prompt_datasettest(args,label_dict,map_dict,savefile):
    ...
    for label in label_dict:
        ...
        if args.task_name=='ag_news':
            retrieve_file = f'/mnt/e72cc1a3-e45e-4a0c-b1e6-0e4357d2e753/zixiao/RAG/self-adaptive-ICL/self-adaptive-ICL-main/output/{args.task_name}/test/1000/retrieved{label}.json'
            ...
            retrieve_file = f'/mnt/e72cc1a3-e45e-4a0c-b1e6-0e4357d2e753/zixiao/RAG/self-adaptive-ICL/self-adaptive-ICL-main/output/{args.task_name}/test/retrieved{label}.json'
    ...
    for j, test in enumerate(solotest_dicts):
        ...
        if args.task_name == 'imdb':
            prompts = [format_s.format(text=test_dict[test]['text'][i][:2500], label=map_dict[int(test_dict[test]['label'][i])]) for i in range(len(test_dict[test]['text']))]
        else:
...
def build_prompt_datasetvalid(args,map_dict,savefile):
    ...
    for label in label_dict:
        '''valid'''
        retrieve_file = f'/mnt/e72cc1a3-e45e-4a0c-b1e6-0e4357d2e753/zixiao/RAG/self-adaptive-ICL/self-adaptive-ICL-main/output/{args.task_name}/valid/1/retrieved{label}.json'
        dftrain = pd.read_csv(f'/mnt/e72cc1a3-e45e-4a0c-b1e6-0e4357d2e753/zixiao/RAG/label-words-are-anchors-main/data/new/train/train_{args.task_name}_{label}.csv')  # replace with your actual path
    ...
    for j, test in enumerate(solotest_dicts):
        row = []
        if args.task_name == 'imdb':
            prompts = [format_s.format(text=test_dict[test]['text'][i][:2500],
...
for args.task_name in [
    ...
    for epoch in epochs:
        ...
        map_dict = load_jsonresult(f'/mnt/e72cc1a3-e45e-4a0c-b1e6-0e4357d2e753/zixiao/RAG/label-words-are-anchors-main/results/logitbased/1shot/labelselect/topk/llama3/{args.task_name}_logiticl_mapdict_{epoch}.json')
        candidate_map_dict = {int(k): ''.join(v) for k, v in map_dict.items()}
        train_save_path = f'/mnt/e72cc1a3-e45e-4a0c-b1e6-0e4357d2e753/zixiao/RAG/label-words-are-anchors-main/data/new/temp/mtopk_{args.task_name}_{epoch}_test.csv'
        build_prompt_datasettest(args, label_dict, candidate_map_dict, train_save_path)
        select_save_path = f'/mnt/e72cc1a3-e45e-4a0c-b1e6-0e4357d2e753/zixiao/RAG/label-words-are-anchors-main/results/logitbased/1shot/labelselect/topk/llama3/test/select_{args.task_name}_{epoch}_test'
        acc, false_list_2 = evaluate(train_save_path, test_csv_path, multi_token, select_save_path, tokenizer, model, args)

        '''valid'''
        train_save_path = f'/mnt/e72cc1a3-e45e-4a0c-b1e6-0e4357d2e753/zixiao/RAG/label-words-are-anchors-main/data/new/temp/topkselect_{args.task_name}_{epoch}_valid.csv'
        map_dict = load_jsonresult(f'/mnt/e72cc1a3-e45e-4a0c-b1e6-0e4357d2e753/zixiao/RAG/label-words-are-anchors-main/results/logitbased/1shot/labelselect/topk/llama3/{args.task_name}_logiticl_mapdict_{epoch}.json')
        candidate_map_dict = {int(k): ''.join(v) for k, v in map_dict.items()}
        build_prompt_datasetvalid(args, candidate_map_dict, train_save_path)
        select_save_path = f'/mnt/e72cc1a3-e45e-4a0c-b1e6-0e4357d2e753/zixiao/RAG/label-words-are-anchors-main/results/logitbased/1shot/labelselect/topk/llama3/valid/select_{args.task_name}_{epoch}_valid'
        acc, false_list_2 = evaluate(train_save_path, test_csv_path, multi_token, select_save_path, tokenizer, model,args)
        ...
        better_label = load_jsonresult(f'/mnt/e72cc1a3-e45e-4a0c-b1e6-0e4357d2e753/zixiao/RAG/label-words-are-anchors-main/results/logitbased/1shot/labelselect/topk/llama3/{args.task_name}_logiticl_mapdict_{epoch}.json')
        ...
        save_file = f'/mnt/e72cc1a3-e45e-4a0c-b1e6-0e4357d2e753/zixiao/RAG/label-words-are-anchors-main/results/logitbased/1shot/labelselect/topk/llama3/valid/select_{args.task_name}_{epoch}_valid'
        test_file =  f'/mnt/e72cc1a3-e45e-4a0c-b1e6-0e4357d2e753/zixiao/RAG/label-words-are-anchors-main/data/new/logitbased/sota/valid_topk_{args.task_name}.csv'
        ...
        with open(f'/mnt/e72cc1a3-e45e-4a0c-b1e6-0e4357d2e753/zixiao/RAG/label-words-are-anchors-main/results/logitbased/1shot/labelselect/topk/llama3/{args.task_name}_logiticl_mapdict_{epoch+1}.json','w') as file:
